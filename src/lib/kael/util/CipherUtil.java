package lib.kael.util;

import java.util.Random;

public class CipherUtil
{
	public CipherUtil() 
	{
		
	}
	protected Random random = new Random();
	protected static final int SOCKET_VER = 0X66;
	protected static final int[] m_SendByteMap = {  };
	protected static final int[] m_RecvByteMap = {  };
	protected int m_cbSendRound = 0;
	protected int m_cbRecvRound = 0;

	protected int m_dwSendXorKey = 0;
	protected int m_dwRecvXorKey = 0;

	protected int m_dwSendPacketCount = 0;

	protected static int g_dwPacketKey = 0xA55AA55A;
	
	/**
	 * 混淆
	 * 
	 * @param wSeed
	 * @return
	 */
	protected static int SeedRandMap(int wSeed) {
		long dwHold = wSeed;
		long aaa = dwHold * 241103L + 2533101L;
		long bbb = 4294967296L;
		aaa %= bbb;
		return (int) (aaa >> 16);
	}
	
	protected static byte MapRecvByte(int cbData) {
		byte cbMap;
		cbMap = (byte) m_RecvByteMap[cbData];
		return cbMap;
	}

	// 映射发送数据 与 发送字节映射表 对应有关系
	protected static byte MapSendByte(int cbData) {
		int len = cbData;
		if (len >= 256) {
			len = len % 256;
		}
		byte cbMap;
		cbMap = (byte) m_SendByteMap[len];
		return cbMap;
	}
	// 转换MD5加密算法
	public static String byteToHexStr(byte[] source) {
		StringBuffer ret = new StringBuffer("");
		for (byte buf : source) {
			String str = Integer.toHexString(buf);
			if (str.length() < 2) {
				ret.append("0" + str);
			} else {
				ret.append(str.substring(str.length() - 2));
			}
		}
		return new String(ret);
	}
	/**
	 * 接收映射
	 * 
	 * @param pcbDataBuffer
	 * @param wDataSize
	 * @return
	 */
	public static byte[] decryptBuffer(byte pcbDataBuffer[]) {

		// 字节反映射
		for (int i = 0; i < pcbDataBuffer.length; i++) {
			if (pcbDataBuffer[i] < 0) {
				pcbDataBuffer[i] = MapRecvByte(256 + pcbDataBuffer[i]);
			} else {
				pcbDataBuffer[i] = MapRecvByte(pcbDataBuffer[i]);
			}
		}
		return pcbDataBuffer;
	}
	/**
	 * 发送映射
	 * 
	 * @param pcbDataBuffer
	 * @param wDataSize
	 * @return
	 */
	public static byte[] encryptBuffer(byte pcbDataBuffer[]) {

		// 字节映射
		for (int i = 0; i < pcbDataBuffer.length; i++) {
			if (pcbDataBuffer[i] < 0) {
				pcbDataBuffer[i] = MapSendByte(256 + pcbDataBuffer[i]);
			} else {
				pcbDataBuffer[i] = MapSendByte(pcbDataBuffer[i]);
			}
		}
		return pcbDataBuffer;
	}
}